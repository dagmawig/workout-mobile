    ...
    function tempList(tList, userTemp) {
        return tList.map((temp, i) => {
            return <TouchableOpacity className='border-[1px] rounded-lg p-1 border-white m-1' key={`${userTemp ? 'user' : 'fixed'}-temp-${i}`} onPress={() => handleShowTemp(temp, userTemp, i)}>
                ...
                <View className='flex-row'><FontAwesome5 name="bell" size={18} color={temp.reminder ? "orange" : "white"} /><Text className='pb-2 italic' style={{ color: temp.reminder ? "orange" : "white" }} >{` ${temp.reminder ? `${Noti.getDayTime(temp)}` : "None"}`}</Text></View>
